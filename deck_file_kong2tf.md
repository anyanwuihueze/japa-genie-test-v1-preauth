...
Convert Kong configuration files to Terraform resources.
...
`-h`, `--help`
:  help for kong2tf (Default: `false`)

`--ignore-credential-changes`
:  Enable flag to add a 'lifecycle' block to each consumer credential, that ignores any changes from local to remote state. (Default: `false`)
