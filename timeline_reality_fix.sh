...
FIX_FILE="timeline_reality_fix_${TIMESTAMP}.txt"

echo "🎯 TIMELINE REALITY FIX PACKAGE - $(date)" > $FIX_FILE
echo "============================================================" >> $FIX_FILE
echo "" >> $FIX_FILE

# Fixed Application Timeline Component
cat >> $FIX_FILE << 'EOT'
============================================================
📝 FILE: src/components/dashboard/application-timeline.tsx
🎯 DESCRIPTION: Replace hardcoded thresholds with real calculation
⚠️  ACTION: Replace entire file
============================================================
...
// REAL PROGRESS CALCULATION - Copy confidence meter approach
const calculateRealTimelineProgress = async (userId: string, userProfile?: any) => {
  try {
    ...
    // 1. Profile Completeness (25% max)
    if (userProfile?.country) realProgress += 8;
    if (userProfile?.destination_country) realProgress += 8;
    if (userProfile?.visa_type) realProgress += 9;

    // 2. Document Readiness (35% max)
    const approvedDocs = documents?.filter(d => d.status === 'approved').length || 0;
    const totalDocs = 8; // Target
    realProgress += (approvedDocs / totalDocs) * 35;

    // 3. AI Engagement (20% max)
    const messageCount = messages?.count || 0;
    if (messageCount >= 10) realProgress += 20;
    else if (messageCount >= 5) realProgress += 15;
    else if (messageCount >= 1) realProgress += 8;

    // 4. Expert Support (10% max)
    const expertCount = expertSessions?.count || 0;
    if (expertCount >= 3) realProgress += 10;
    else if (expertCount >= 1) realProgress += 5;

    // 5. Existing Progress (10% max)
    const existingProgress = progress?.progress_percentage || 0;
    realProgress += Math.min(existingProgress * 0.1, 10);

    return Math.min(Math.round(realProgress), 100);
  } catch (error) {
    console.error('Error calculating real timeline progress:', error);
    return 0; // Conservative fallback
  }
};

// DYNAMIC STAGE THRESHOLDS - Based on real user actions
const getDynamicStageThresholds = (realProgress: number, profile: any, documents: any[], messages: any[]) => {
  const approvedDocs = documents?.filter(d => d.status === 'approved').length || 0;
  const messageCount = messages?.count || 0;

  return {
    // Stage 1: Getting Started (0-25%)
    onboarding: {
      completed: realProgress >= 25,
      inProgress: realProgress >= 10,
      // Completed when: Profile complete + some engagement
      isCompleted: !!(profile?.country && profile?.destination_country && profile?.visa_type && messageCount >= 1)
    },
    
    // Stage 2: Document Preparation (25-50%)
    documentPrep: {
      completed: realProgress >= 50,
      inProgress: realProgress >= 30,
      // Completed when: 4+ docs approved + active engagement
      isCompleted: approvedDocs >= 4 && messageCount >= 3
    },
    
    // Stage 3: Application Submission (50-75%)
    submission: {
      completed: realProgress >= 75,
      inProgress: realProgress >= 60,
      // Completed when: 6+ docs + expert consultation
      isCompleted: approvedDocs >= 6 && messageCount >= 5
    },
    
    // Stage 4: Interview Prep (75-90%)
    interview: {
      completed: realProgress >= 90,
      inProgress: realProgress >= 80,
      // Completed when: Almost ready + expert sessions
      isCompleted: realProgress >= 88
    },
    
    // Stage 5: Final Prep (90-100%)
    finalPrep: {
      completed: realProgress >= 100,
      inProgress: realProgress >= 95,
      isCompleted: realProgress >= 98
    },
    
    // Stage 6: Approval (100%)
    approval: {
      completed: realProgress >= 100,
...
export function ApplicationTimeline({ userId, userProfile, currentProgress, className }: ApplicationTimelineProps) {
  ...
  useEffect(() => {
    const fetchTimelineData = async () => {
      try {
        ...
        // Fetch REAL user data for calculations
        const [
          { data: timelineData },
          { data: progressData },
          { data: messages },
          ...
          supabase.from('user_timeline').select('*').eq('user_id', userId).single(),
          supabase.from('user_progress').select('*').eq('user_id', userId).single(),
          supabase.from('messages').select('count').eq('user_id', userId).single(),
          supabase.from('user_documents').select('status').eq('user_id', userId)
        ]);

        // Calculate REAL progress (not hardcoded!)
        const calculatedProgress = await calculateRealTimelineProgress(userId, userProfile);
        setRealProgress(calculatedProgress);
        
        // Get dynamic thresholds based on REAL data
        const thresholds = getDynamicStageThresholds(calculatedProgress, userProfile, documents, messages);
        
        // Generate timeline with REAL calculations
        const generatedStages = generateTimelineStages(userProfile, progressData, calculatedProgress, thresholds, documents, messages);
  ...
  const generateTimelineStages = (profile: any, progress: any, overallProgress: number, thresholds: any, documents: any[], messages: any[]): TimelineStage[] => {
    const approvedDocs = documents?.filter(d => d.status === 'approved').length || 0;
    const messageCount = messages?.count || 0;
    ...
    const stages: TimelineStage[] = [
      {
        ...
        status: thresholds.onboarding.completed ? 'completed' : thresholds.onboarding.inProgress ? 'in-progress' : 'pending',
        order: 1,
        ...
        progress: Math.min(overallProgress, 25), // Cap at 25% for stage 1
        subtasks: [
          ...
          { id: 'first-message', title: 'Start AI conversation', completed: messageCount >= 1, required: true }
        ]
      },
      {
        ...
        status: thresholds.documentPrep.completed ? 'completed' : thresholds.documentPrep.inProgress ? 'in-progress' : 'pending',
        order: 2,
        ...
        progress: overallProgress >= 25 ? Math.min(((overallProgress - 25) / 25) * 100, 100) : 0, // 25-50% range
        subtasks: [
          ...
          { id: 'ai-engagement', title: 'Use AI document checker', completed: messageCount >= 3, required: false }
        ]
      },
      {
        ...
        status: thresholds.submission.completed ? 'completed' : thresholds.submission.inProgress ? 'in-progress' : 'pending',
        order: 3,
        ...
        progress: overallProgress >= 50 ? Math.min(((overallProgress - 50) / 25) * 100, 100) : 0, // 50-75% range
        subtasks: [
          { id: 'forms-completed', title: 'Complete application forms', completed: overallProgress >= 65, required: true },
          { id: 'documents-uploaded', title: 'Upload all documents', completed: approvedDocs >= 6, required: true },
          { id: 'expert-review', title: 'Get expert review', completed: messageCount >= 5, required: false },
          { id: 'fees-paid', title: 'Pay application fees', completed: overallProgress >= 70, required: true }
        ]
      },
      {
        ...
        status: thresholds.interview.completed ? 'completed' : thresholds.interview.inProgress ? 'in-progress' : 'pending',
        order: 4,
        ...
        progress: overallProgress >= 75 ? Math.min(((overallProgress - 75) / 15) * 100, 100) : 0, // 75-90% range
        subtasks: [
          { id: 'interview-scheduled', title: 'Schedule interview', completed: overallProgress >= 85, required: true },
          { id: 'mock-interview', title: 'Complete mock interview', completed: messageCount >= 8, required: true },
          { id: 'questions-practiced', title: 'Practice common questions', completed: overallProgress >= 87, required: true },
          { id: 'expert-feedback', title: 'Get expert feedback', completed: messageCount >= 10, required: false }
        ]
      },
      {
        ...
        status: thresholds.finalPrep.completed ? 'completed' : thresholds.finalPrep.inProgress ? 'in-progress' : 'pending',
        order: 5,
        ...
        progress: overallProgress >= 90 ? Math.min(((overallProgress - 90) / 10) * 100, 100) : 0, // 90-100% range
        subtasks: [
          { id: 'final-check', title: 'Final document check', completed: overallProgress >= 95, required: true },
          { id: 'travel-ready', title: 'Prepare for travel', completed: overallProgress >= 97, required: false },
          { id: 'backup-plan', title: 'Have backup plan', completed: overallProgress >= 98, required: false }
        ]
      },
      {
        id: 'visa-approval',
        ...
        status: thresholds.approval.completed ? 'completed' : 'pending',
        order: 6,
        estimatedDuration: '1-2 weeks',
  ...
  return (
    <Card className={`${className} ${isMobile ? 'shadow-sm' : 'shadow-lg'}`}>
      <CardHeader className={`${isMobile ? 'p-4' : 'p-6'} pb-4`}>
        ...
        {/* REAL Overall Progress - Not hardcoded! */}
...
echo ""
echo "============================================================" >> $FIX_FILE
echo "🧪 TESTING COMMANDS" >> $FIX_FILE
echo "============================================================" >> $FIX_FILE
echo "" >> $FIX_FILE
echo "# After replacing the file, run these commands:" >> $FIX_FILE
echo "npx tsc --noEmit" >> $FIX_FILE
echo "" >> $FIX_FILE
echo "# Verify magic numbers are GONE:" >> $FIX_FILE
echo 'grep -r "overallProgress >= 20\|overallProgress >= 5\|overallProgress >= 50" src/' >> $FIX_FILE
echo "" >> $FIX_FILE
echo "# Should show NO RESULTS - if you see any, the fix didn't apply" >> $FIX_FILE
echo "" >> $FIX_FILE
echo "# Test with real user actions:" >> $FIX_FILE
echo "1. Upload a document → Timeline should advance" >> $FIX_FILE
echo "2. Send chat messages → Progress should increase" >> $FIX_FILE  
echo "3. Complete profile → Stage should become 'completed'" >> $FIX_FILE
echo "" >> $FIX_FILE
echo "# Success indicators:" >> $FIX_FILE
echo "- Timeline shows real percentage (not 17%)" >> $FIX_FILE
echo "- Stages unlock based on actions (not magic numbers)" >> $FIX_FILE
echo "- Console shows 'Real progress calculated: X%'" >> $FIX_FILE

echo ""
echo "✅ Fix package created!"
echo "📁 File: timeline_reality_fix_${TIMESTAMP}.txt"
echo "📊 Size: $(wc -c < timeline_reality_fix_${TIMESTAMP}.txt) bytes"
