...
const iconsPath = path.join(__dirname, '../../src/components/icons.tsx');
let content = fs.readFileSync(iconsPath, 'utf8');

// Extract just the JapaGenieLogo SVG (excluding React wrapper)
const logoMatch = content.match(/<svg[\s\S]*?<\/svg>/);
if (logoMatch) {
  let svgContent = logoMatch[0];
  ...
  svgContent = svgContent
    ...
    .replace(/\{([^}]+)\}/g, (match, p1) => {
      ...
      if (p1.includes('stopColor')) {
        return p1.replace(/'/g, '"').replace(/stopColor:/g, 'stop-color:');
      }
      return match;
    })
