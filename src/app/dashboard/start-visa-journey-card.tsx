...
import { Target, Map, CheckCircle, AlertCircle } from 'lucide-react';
...
export function StartVisaJourneyCard({ className, onStartJourney }: StartVisaJourneyCardProps) {
  ...
  const { userProfile, progressPercentage, messageCount, documentCount, userProgress, loading, error } = useDashboardData();
  ...
  const realProgress = calculateRealBaseline(userProfile, messageCount, documentCount);

  // ✅ FIXED: Check userProgress.journey_started instead of realProgress.journey_started
  if (userProgress?.journey_started) {
    return (
      <Card className={`${className} ${isMobile ? 'shadow-sm' : 'shadow-lg'}`}>
        <CardHeader className={`${isMobile ? 'p-4' : 'p-6'}`}>
          ...
          <CardDescription className={`${isMobile ? 'text-sm' : 'text-base'}`}>
            Track your progress to {userProfile?.destination_country || 'your destination'}
          </CardDescription>
        </CardHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">{userProfile?.destination_country ? '✓' : '✗'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Visa Type Selected</span>
                <span className="font-medium">{userProfile?.visa_type ? '✓' : '✗'}</span>
              </div>
            </div>
  ...
  return (
    <Card className={`${className} ${isMobile ? 'shadow-sm' : 'shadow-lg'} border-2 border-blue-200`}>
      <CardHeader className={`${isMobile ? 'p-4' : 'p-6'} bg-gradient-to-r from-blue-50 to-purple-50`}>
        ...
        <CardDescription className={`${isMobile ? 'text-sm' : 'text-base'}`}>
          {realProgress.progressPercentage}% complete - Lock in your visa path to {userProfile?.destination_country}
        </CardDescription>
      </CardHeader>
      <CardContent className={`${isMobile ? 'p-4' : 'p-6'} pt-0`}>
        <div className="space-y-4">
...
function calculateRealBaseline(profile: any, messageCount: number, documentCount: number) {
