...
import { Upload, Camera, FileText, Loader2, CheckCircle, AlertTriangle, XCircle, X } from 'lucide-react';
  ...
  const handleAnalyze = async (retryWithCamera = false) => {
    ...
    try {
      ...
      const response = await fetch('/api/document-check', {
        ...
        body: JSON.stringify({ documentDataUri: dataUri, userId: user.id }),
      });
        ...
        <CardContent className="space-y-4">
          ...
          {file && (
            <div className="flex items-center gap-2 p-3 bg-muted rounded-lg">
              ...
              <Button variant="ghost" size="sm" onClick={() => { setFile(null); setResult(null); setError(null); setCameraError(null); }}>
                Remove
              </Button>
            </div>
          )}
            ...
            <Alert variant="destructive">
              ...
              <AlertDescription className="flex items-center justify-between">
                ...
                {error.includes('PDF conversion failed') && (
                  <Button variant="outline" size="sm" onClick={() => handleAnalyze(true)} className="ml-2">
          ...
          <Button onClick={() => handleAnalyze(false)} disabled={!file || loading} className="w-full" size="lg">
            {converting ? (
              <><Loader2 className="animate-spin mr-2" /> Converting PDF...</>
            ) : loading ? (
        ...
        <Card className={result.overallStatus === 'pass' ? 'border-green-200 bg-green-50' : result.overallStatus === 'warning' ? 'border-yellow-200 bg-yellow-50' : 'border-red-200 bg-red-50'}>
          ...
          <CardContent className="space-y-6">
              ...
              <div>
                ...
                {result.criticalIssues.map((issue, i) => (
                  <div key={i} className="bg-white p-4 rounded mb-2 border border-red-200">
                    ...
                    <p className="text-sm text-green-700"><strong>Fix:</strong> {issue.recommendation}</p>
                  </div>
                ))}
              </div>
            )}
              ...
              <div>
                ...
                {result.warnings.map((w, i) => (
                  <div key={i} className="bg-white p-4 rounded mb-2 border border-yellow-200">
                    ...
                    <p className="text-sm text-green-700"><strong>Fix:</strong> {w.recommendation}</p>
                  </div>
                ))}
              </div>
            )}
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {result.passed.map((check, i) => (
                    <li key={i} className="flex items-start"><CheckCircle className="w-4 h-4 text-green-600 mr-2 mt-0.5" /> <span className="text-sm">{check}</span></li>
                  ))}
                </ul>
              </div>
            )}
            ...
            <div className="flex gap-4">
              <Button onClick={() => { setFile(null); setResult(null); setError(null); setCameraError(null); }} variant="outline" className="flex-1">Analyze Another</Button>
              <Button onClick={() => window.print()} className="flex-1">Print Report</Button>
            </div>
          </CardContent>
        </Card>
      )}
