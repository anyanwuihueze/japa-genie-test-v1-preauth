...
export default function UserChat() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (chatResult.insights) setInsights(chatResult.insights);
      ...
      <div className="w-full md:w-1/2 flex flex-col border-r border-gray-200 bg-white overflow-y-auto">
          ...
          <div className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 border-b">
            <div className="max-w-2xl mx-auto">
              <h3 className="font-semibold text-sm mb-2 flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-amber-500" />Make Your 3 Wishes Count!
              </h3>
        ...
        <div className="bg-blue-50 py-1.5 px-4 text-center text-xs text-blue-700 font-medium flex items-center justify-between">
          ...
          {user && messages.length > 1 && (
            <Button variant="ghost" size="sm" onClick={handleClearChat} disabled={isClearing} className="h-6 px-2 text-xs">
              <Trash2 className="w-3 h-3 mr-1" />{isClearing ? 'Clearing...' : 'Clear Chat'}
            </Button>
          )}
        </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-3">
          {messages.map((msg, idx) => (
            <div key={msg.id || idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-xs lg:max-w-md px-4 py-2.5 rounded-lg text-sm ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white text-gray-800 border rounded-bl-none shadow'}`}>
        ...
        <div className="p-4 border-t bg-white/80 backdrop-blur">
          <div className="flex items-center justify-between mb-2">
            <Badge variant={wishesLeft > 0 ? "default" : "secondary"}>{user ? '✨ Unlimited wishes' : wishesLeft > 0 ? `${wishesLeft} wish${wishesLeft !== 1 ? 'es' : ''} left` : 'All wishes used'}</Badge>
            <div className="text-xs text-gray-500">{user ? `Wish ${wishCount}` : `Wish ${wishCount} of ${MAX_WISHES}`}</div>
          </div>
          <form onSubmit={handleSubmit} className="flex space-x-2">
            <input ref={inputRef} type="text" value={currentInput} onChange={(e) => setCurrentInput(e.target.value)} placeholder={user ? "Ask your next wish..." : "Ask for your first wish..."} className="flex-1 p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={isTyping || (!user && wishCount >= MAX_WISHES)} />
            <button type="submit" disabled={isTyping || !currentInput.trim() || (!user && wishCount >= MAX_WISHES)} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white font-medium rounded-lg">{isTyping ? '...' : 'Send'}</button>
          </form>
          {!user && wishCount >= MAX_WISHES && <div className="mt-3"><CheckoutButton /></div>}
        </div>
      </div>

      <div className="w-full md:w-1/2 bg-gradient-to-b from-blue-50 to-purple-50 p-6 overflow-y-auto">
        ...
        {isTyping ? (
          <div className="space-y-4"><div className="h-4 bg-gray-200 rounded animate-pulse"></div><div className="h-4 bg-gray-200 rounded animate-pulse w-3/4"></div></div>
        ) : insights?.suggestedCountries?.length ? (
          <div className="space-y-6">
            <div>
              <h4 className="font-semibold text-gray-700 mb-3">Country Comparison</h4>
              <div className="bg-white p-4 rounded-lg shadow">
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={insights.suggestedCountries.map(c => ({ name: c.name, Cost: c.estimatedCost, Time: c.processingTimeMonths }))}>
                    <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis yAxisId="left" orientation="left" stroke="#8884d8" /><YAxis yAxisId="right" orientation="right" stroke="#82ca9d" /><Tooltip /><Legend />
                    <Bar yAxisId="left" dataKey="Cost" fill="#8884d8" name="Cost (USD)" /><Bar yAxisId="right" dataKey="Time" fill="#82ca9d" name="Months" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            {insights.suggestedCountries.map((country, idx) => (
              <div key={idx} className="bg-white p-4 rounded-lg shadow space-y-2">
                ...
                <p className="text-sm text-blue-600">{country.visaType}</p>
                <div className="text-xs text-gray-600"><p>💰 ${country.estimatedCost.toLocaleString()}</p><p>⏱️ {country.processingTimeMonths} months</p></div>
                <div><p className="text-xs font-semibold text-green-700">Pros:</p><ul className="text-xs">{country.pros.map((p, i) => <li key={i} className="flex gap-1"><span className="text-green-500">✓</span>{p}</li>)}</ul></div>
                <div><p className="text-xs font-semibold text-red-700">Cons:</p><ul className="text-xs">{country.cons.map((c, i) => <li key={i} className="flex gap-1"><span className="text-red-500">✗</span>{c}</li>)}</ul></div>
              </div>
            ))}

            {insights.timeline?.map((step, idx) => (
              <div key={idx} className="flex gap-3">
                <div className="w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center font-bold">{idx + 1}</div>
                <div className="flex-1 bg-white p-3 rounded-lg shadow"><p className="text-sm font-medium">{step.step}</p><p className="text-xs text-gray-500">~{step.durationWeeks} weeks</p></div>
              </div>
            ))}

            {insights.alternativeStrategies?.map((strategy, idx) => (
              <div key={idx} className="flex gap-2 bg-yellow-50 p-3 rounded-lg"><span className="text-yellow-500">💡</span><span className="text-sm">{strategy}</span></div>
            ))}
          </div>
        ) : (
          <div className="text-center py-8"><Sparkles className="w-10 h-10 mx-auto text-gray-300 mb-3" /><p className="text-gray-500 text-sm">Ask a visa-related question to see insights.</p></div>
        )}
      </div>
    </div>
  );
