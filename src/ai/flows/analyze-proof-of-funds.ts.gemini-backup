...
IMPORTANT: Return ONLY the JSON object, no markdown formatting, no code blocks, no explanations.`;
    ...
    // Check if AI actually analyzed (not just returning example values)
    if (analysis.summary.totalScore === 7 && 
        analysis.financialAnalysis?.totalAssets === 25000 &&
        analysis.summary.confidence === 85) {
      console.warn('⚠️ AI may have returned example values instead of real analysis');
    }
