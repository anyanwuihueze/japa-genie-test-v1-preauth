...
interface NextBestAction {
  ...
  estimatedTime: string; // "5 mins", "1 hour", "1 day"
  actionUrl: string;
  actionText: string;
...
export function NextBestAction({ userId, userProfile, currentProgress, className }: NextBestActionProps) {
  ...
  const calculateNextBestActions = async (userData: any): Promise<NextBestAction[]> => {
    ...
    if (currentProgress > 50 && expertSessions === 0) {
      actions.push({
    ...
    // MEDIUM PRIORITY - Timeline Planning
    if (currentProgress > 30 && !userProfile?.preferred_start_date) {
      actions.push({
        id: 'set-timeline',
        title: 'Set Your Timeline',
        description: 'Define your preferred travel date to get a personalized application schedule',
        priority: 'medium',
        category: 'timeline',
        estimatedImpact: 75,
        estimatedTime: '5 mins',
        actionUrl: '/timeline',
        actionText: 'Set Timeline',
        whyImportant: 'Proper timeline planning prevents last-minute rush and mistakes',
        smartTip: 'Start 6-8 months before your intended travel date for best results',
        benefits: ['Structured plan', 'Deadline reminders', 'Stress reduction']
      });
    }

    // MEDIUM PRIORITY - Financial Planning
    if (currentProgress > 40 && !userProfile?.funds_available) {
      actions.push({
        id: 'financial-planning',
        title: 'Complete Financial Assessment',
        description: 'Document your financial readiness for the visa application',
        priority: 'medium',
        category: 'financial',
        estimatedImpact: 70,
        estimatedTime: '10 mins',
        actionUrl: '/proof-of-funds',
        actionText: 'Assessment',
        whyImportant: 'Financial requirements are the #1 reason for visa rejections',
        smartTip: 'We\'ll help you understand exactly how much you need to show',
        benefits: ['Clear requirements', 'Rejection prevention', 'Peace of mind']
      });
    }

    // LOW PRIORITY - Expert Consultation
    if (currentProgress > 60 && expertSessions < 2) {
      actions.push({
        id: 'expert-consultation',
        title: 'Book Expert Consultation',
        description: 'Get personalized guidance from visa consultants with 95% success rate',
        priority: 'low',
        category: 'expert',
        estimatedImpact: 85,
        estimatedTime: '1 hour',
        actionUrl: '/experts',
        actionText: 'Find Expert',
        whyImportant: 'Professional review catches issues you might miss',
        smartTip: 'Our experts have 15+ years experience with your destination country',
        benefits: ['Professional review', 'Insider tips', 'Application optimization']
      });
    }
