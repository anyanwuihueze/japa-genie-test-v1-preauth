...
export function HowItWorksAnimated() {
  ...
  return (
    <section ref={sectionRef} className="relative py-20 overflow-hidden bg-gradient-to-b from-blue-50 to-white">
      {/* ===== ONE BIG, BRIGHT, SLOW ORB ===== */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div 
          className="absolute w-64 h-64 bg-gradient-to-br from-purple-400 via-pink-400 to-yellow-400 rounded-full opacity-90 blur-3xl"
          style={{
            left: '50%',
            top: '30%',
      ...
      <div className="relative z-10 container mx-auto px-4">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-black rounded-[2rem] p-3 shadow-xl w-80">
            <div className="bg-white rounded-[1.5rem] overflow-hidden flex flex-col h-[34rem]">
              <div className="flex items-center justify-between px-4 py-2 text-xs text-gray-500 border-b"><span>9:41 AM</span><div className="flex space-x-1"><div className="w-1 h-1 bg-gray-400 rounded-full" /><div className="w-1 h-1 bg-gray-400 rounded-full" /><div className="w-1 h-1 bg-gray-400 rounded-full" /></div></div>
              <div className="flex items-center space-x-3 px-4 py-3 border-b"><div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center"><Sparkles className="w-4 h-4 text-white" /></div><div><p className="font-semibold text-sm">Japa Genie</p><p className="text-xs text-green-500">Online</p></div></div>
                ...
                {visibleMsgs.map((m, i) => (
                  <div key={i} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[75%] rounded-2xl px-4 py-2 text-sm ${m.sender === 'user' ? 'bg-blue-500 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none shadow'}`}>
                      ...
                      {i === visibleMsgs.length - 1 && <span className="inline-block w-1 h-4 bg-current animate-pulse ml-1" />}
                    </div>
                  </div>
                ))}
                {msgIndex < currentConv.messages.length - 1 && (
                  <div className="flex justify-start"><div className="bg-white rounded-2xl px-3 py-2 shadow"><div className="flex space-x-1"><div className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} /><div className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} /><div className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} /></div></div></div>
                )}
              </div>
              <div className="px-4 py-3 border-t bg-white"><div className="flex space-x-2"><input type="text" placeholder="Type a message…" className="flex-1 bg-gray-100 rounded-full px-4 py-2 text-sm focus:outline-none" disabled /><button className="p-2 bg-blue-500 text-white rounded-full"><Sparkles className="w-4 h-4" /></button></div></div>
            </div>
          </div>
        </div>
        ...
        <div className="text-center"><Button asChild size="lg"><Link href="/chat">Start Your Journey <Sparkles className="ml-2 w-4 h-4" /></Link></Button></div>
      </div>
    </section>
  );
}